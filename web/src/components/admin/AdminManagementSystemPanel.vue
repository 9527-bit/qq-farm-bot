...
import type { CaptureConfig, LoginLinks, SystemConfig, WxConfig } from '@/composables/useAdminManagementConfig'
...
defineModel<WxConfig>('localWxConfig', { required: true })
...
<template>
  <div class="space-y-4">
    ...
    <div class="rounded-2xl bg-gray-50 px-4 py-3 text-sm text-gray-600 dark:bg-gray-900/40 dark:text-gray-300">
      修改后会影响登录页、全局连接参数与抓包服务，保存前建议再次核对目标环境。
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="border border-gray-200 rounded-lg bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-300">
          当前使用内置应用宝扫码协议。旧版微信 API 配置仅保留用于数据兼容，不控制新版扫码行为；普通用户账号隔离始终开启。
        </p>
      </div>
